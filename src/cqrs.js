(function(global) {
    ...
    function getQuery(queryName) {
        ...
        throw new Error('unable to find the query ' + commandName);
    }
    ...
    function cqrs(params) {
        ...
        function when(commandName) {
            ...
            function invoke(callback) {
                if (cqrs.debug) {
                    console.log('cqrs - when - add handler %s:%s', owner, cmdName);
                }
        ...
        function publish(eventName, payload, metadata) {
            ...
            var promises = listeners.map(function(listener) {
                return new Promise(function(resolve) {
                    resolve(listener.callback(payload, metadata));
                });
            });
        ...
        function on(eventName) {
            ...
            function invoke(callback) {
                if (cqrs.debug) {
                    console.log('cqrs - on - add listener %s:%s', owner, evtName);
                }
        ...
        function calling(queryName) {
            ...
            function invoke(queryFunction) {
                if (cqrs.debug) {
                    console.log('cqrs - calling - add query %s:%s', owner, qryName);
                }
        ...
        function call() {
            ...
            var qryName = generateTechnicalName(namespace, 'qry', queryName);
            if (cqrs.debug) {
                console.log('cqrs - call - call query %s:%s', owner, qryName);
            }
            return new Promise(function(resolve, reject) {
                var query = getQuery(qryName);
                resolve(query.queryFunction.apply(global, args));
            });
        }
        exports.call = call;
            ...
            function aggregateWhen(commandName) {
                ...
                function invoke(callback) {
                    ...
                    function apply(eventName) {
                        ...
                        if (cqrs.debug) {
                            console.log('cqrs - aggregate when - add handler %s:%s', owner, cmdName);
                        }
